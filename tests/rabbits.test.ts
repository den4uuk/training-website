...
import { Hedgehog } from '../src/models/rabbit';
...
// Тести API вебдодатку сайту про їжаків
describe('API вебдодатку сайту про їжаків', () => {
    ...
    // Перед кожним тестом очищуємо колекцію їжаків
    beforeEach(async () => {
        await Hedgehog.deleteMany({});
    });

    // Тести для створення запису про нового їжака (POST-запит)
    describe('POST /api/rabbits', () => {
        it('має створити запис про нового їжака', done => {
            // Тестові дані їжака
            const hedgehog = {
                name: 'Вухань',
                age: 2,
                ...
                description: 'Сірий їжак',
            };

            // Виконуємо POST-запит для створення запису про їжака
            chai.request(app)
                .post('/api/rabbits')
                .send(hedgehog)
                .end((err, res) => {
                    ...
                    expect(res.body).to.have.property('name', hedgehog.name);
                    expect(res.body).to.have.property('age', hedgehog.age);
                    expect(res.body).to.have.property('height', hedgehog.height);
                    expect(res.body).to.have.property('weight', hedgehog.weight);
                    expect(res.body).to.have.property('gender', hedgehog.gender);
                    expect(res.body).to.have.property('description', hedgehog.description);
    ...
    // Тести для отримання всіх записів їжаків (GET-запит)
    describe('GET /api/rabbits', () => {
        it('має отримати всіх їжаків', async () => {
            // Створюємо тестовий запис їжака
            const testHedgehog = new Hedgehog({
                name: 'Білан',
                age: 3,
                height: 35,
                weight: 3.2,
                gender: 'male',
                description: 'Білий їжак',
            });
            await testHedgehog.save();

            // Виконуємо GET-запит для отримання всіх записів їжаків
            ...
            expect(res.body[0]).to.have.property('description', 'Білий їжак');
            expect(res.body[0]).to.have.property('dateAdded');
            expect(new Date(res.body[0].dateAdded)).to.be.instanceOf(Date);
        });
    });

    // Тести для отримання запису конкретного їжака за ID (GET-запит)
    describe('GET /api/rabbits/:id', () => {
        it('має отримати конкретного їжака за id', async () => {
            // Створюємо запис тестового їжака
            const testHedgehog = new Hedgehog({
                name: 'Косий',
                age: 1,
                height: 25,
                weight: 1.8,
                gender: 'male',
                description: 'Коричневий їжак',
            });
            const savedHedgehog = await testHedgehog.save();

            // Виконуємо GET-запит для отримання запису їжака за ID
            const res = await chai.request(app).get(`/api/rabbits/${String(savedHedgehog._id)}`);
            expect(res).to.have.status(200);
            ...
            expect(res.body).to.have.property('description', 'Коричневий їжак');
        });

        it('має повернути 404 для неіснуючого їжака', async () => {
            // Виконуємо GET-запит для неіснуючого ID їжака
            const res = await chai.request(app).get('/api/rabbits/654321654321654321654321');
            expect(res).to.have.status(404);
        });
    });

    // Тести для повного оновлення запису про їжака (PUT-запит)
    describe('PUT /api/rabbits/:id', () => {
        it('має повністю оновити запис про їжака', async () => {
            // Створюємо тестового їжака
            const testHedgehog = new Hedgehog({
                name: 'Оригінальний',
                age: 1,
                height: 25,
            ...
            // Виконуємо PUT-запит для повного оновлення запису про їжака
            const res = await chai
                .request(app)
                .put(`/api/rabbits/${String(savedHedgehog._id)}`)
                .send(updatedData);
        ...
        it("має завершитися невдачею при відсутності обов'язкових полів", async () => {
            ...
            // Перевіряємо, що їжак не змінився
            const unchangedHedgehog = await Hedgehog.findById(savedHedgehog._id);
            expect(unchangedHedgehog).to.have.property('name', 'Оригінальний');
            expect(unchangedHedgehog).to.have.property('height', 25);
            expect(unchangedHedgehog).to.have.property('weight', 1.8);
        });
    });

    // Тести для часткового оновлення запису про їжака (PATCH-запит)
    describe('PATCH /api/rabbits/:id', () => {
        it('має частково оновити запис про їжака', async () => {
            // Створюємо тестового їжака
            const testHedgehog = new Hedgehog({
                name: 'Оригінальний',
                age: 1,
            ...
            const res = await chai
                .request(app)
                .patch(`/api/rabbits/${String(savedHedgehog._id)}`)
                .send(patchData);
    ...
    // Тести для видалення запису їжака (DELETE-запит)
    describe('DELETE /api/rabbits/:id', () => {
        it('має видалити запис про їжака', async () => {
            // Створюємо тестового їжака
            const testHedgehog = new Hedgehog({
                name: 'Стрибунець',
                age: 2,
                ...
                weight: 2.1,
                gender: 'female',
                description: 'Чорний їжак',
            });
            const savedHedgehog = await testHedgehog.save();

            // Виконуємо DELETE-запит
            const res = await chai.request(app).delete(`/api/rabbits/${String(savedHedgehog._id)}`);
            expect(res).to.have.status(200);
            expect(res.body).to.have.property('message', 'Запис про їжака видалено');

            // Перевіряємо, що запис про їжака дійсно видалено з бази
            const findHedgehog = await Hedgehog.findById(savedHedgehog._id);
            expect(findHedgehog).to.be.null;
        });
    });
});
